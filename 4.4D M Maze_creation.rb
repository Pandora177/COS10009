...
# left to right. The right click on a cell for the program to find a
...
class GameWindow < Gosu::Window
 ...
 # nested while loop inside.
   column_index = 0
    ...
    while (row_index < y_cell_count)
        ...
        puts("Cell x: #{column_index} y: #{row_index} north: #{@north} south: #{@south} east: #{@east} west: #{@west}" )
        row_index += 1
        end
        puts("--------End of Column--------")
        column_index += 1
 ...
 # But you DO need to complete it later for the Maze Search task
    ...
    [[cell_x,cell_y]] # We are at the east wall - exit
        ...
        south_path = nil
    if (ARGV.length > 0) # debug
        puts "Searching. In cell x: " + cell_x.to_s + " y: " +
        cell_y.to_s
    end
 ...
 # A path was found:
    ...
    def button_down(id)
        ...
        if (ARGV.length > 0) # debug
            puts("Cell clicked on is x: " + cell[0].to_s + " y: " +
        cell[1].to_s)
         end
    ...
    def draw
        ...
        while (row_index < y_cell_count)
            ...
            Gosu.draw_rect(column_index * CELL_DIM, row_index * CELL_DIM,
            CELL_DIM, CELL_DIM, color, ZOrder::TOP, mode=:default)
            row_index += 1
        end
        column_index += 1
        end
