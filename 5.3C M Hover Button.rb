...
class DemoWindow < Gosu::Window
  ...
  def draw()
    ...
    Gosu.draw_rect(49, 49, 103, 53, @bg_btn, ZOrder::TOP, mode=:default)
    # Draw the button
 ...
 # This still needs to be fixed!
  ...
  def mouse_over_button(mouse_x, mouse_y)
    if mouse_x.between?(50, 150) && mouse_y.between?(50, 100)
      true
    else
      false
    end
  ...
  def update
      if mouse_over_button(mouse_x, mouse_y)
        @bg_btn = Gosu::Color::BLACK
      else
        @bg_btn = Gosu::Color::WHITE
     end
    end
